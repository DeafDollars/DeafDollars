...
uint256 hashGenesisBlock("0xed512fd39debf3345b579620f517f569e9d62e8c7cde36f0af84c5615ab99680");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // DEAFDOLLARS: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "DEAFDOLLARS Signed Message:\n";
...
{
    // DEAFDOLLARS: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    if(nHeight == 1){
        nSubsidy = nSubsidy * 466600 * 10  / 100 / 1;
     }
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // DEAFDOLLARS: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // DEAFDOLLARS: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // DEAFDOLLARS: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xed512fd39debf3345b579620f517f569e9d62e8c7cde36f0af84c5615ab99680");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1413407650;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084622407;
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("DEAFDOLLARSMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("DEAFDOLLARSMiner : ProcessBlock, block not accepted");
    }
...
void static LitecoinMiner(CWallet *pwallet)
{
    ...
    try { loop {
        ...
        printf("Running DEAFDOLLARS with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("DEAFDOLLARSMiner terminated\n");
        throw;
    }
}
